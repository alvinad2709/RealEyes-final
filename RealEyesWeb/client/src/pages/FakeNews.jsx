...
export default function FakeNews() {
  ...
  const startAnalysis = async () => {
    ...
    try {
      ...
      console.error(err);
      setError(err.response?.data?.message || err.message || "Failed to reach Deepguard Backend");
    } finally {
       setAnalyzing(false);
    }
  };
      ...
      <div className={clsx("glass-panel p-6 border transition-all duration-500", 
        ...
        {!results && !analyzing && (
           <div className="h-full flex flex-col items-center justify-center text-center space-y-4">
             ...
             <p className="text-textMuted text-sm">Provide news text or a URL and let Deepguard's LLaMA engine dissect it.</p>
           </div>
        )}
          ...
          <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="space-y-3">
               <div className="text-xs font-mono text-textMuted uppercase tracking-wider flex items-center gap-2">
                 <Zap className="w-3 h-3 text-yellow-500" /> Deepguard LLaMA-3 Diagnostics
               </div>
